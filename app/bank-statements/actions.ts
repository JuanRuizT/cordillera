...
export async function moveToAccounting(
  ...
  // A single bulk insert instead of N sequential creates inside an interactive transaction —
  // with a large statement, the per-query round trips over the Neon HTTP driver adapter could
  // add up past Prisma's 5s interactive-transaction timeout. One INSERT is still all-or-nothing.
