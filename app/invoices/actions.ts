...
import type { PaymentProofRow } from "../accounting/actions"
...
export type InvoiceData = {
  ...
  fileUrl: string | null
  notes: string | null
  abonos: Array<{ id: string; date: string; amount: string; voucherNumber: number | null; accountingRecordId: string | null; hasPaymentProof: boolean }>
}
...
function toInvoiceData(inv: InvoiceWithAbonos): InvoiceData {
  ...
  return {
    ...
    abonos: inv.abonos.map((a) => ({
      ...
      accountingRecordId: a.accountingRecordId,
      hasPaymentProof: (a.accountingRecord?.paymentProofs.length ?? 0) > 0,
    })),
  }
}
    ...
    include: {
      ...
      abonos: {
        select: {
          ...
          date: true,
          amount: true,
          accountingRecordId: true,
          accountingRecord: {
            select: {
              date: true,
              expenses: true,
              expenseVoucher: { select: { number: true } },
...
export async function listInvoiceAbonos(invoiceId: string): Promise<AbonoRow[]> {
  ...
  const abonos = await prisma.abono.findMany({
    ...
    include: {
      accountingRecord: {
        include: {
          expenseVoucher: { select: { number: true } },
          paymentProofs: { select: { id: true, fileName: true }, orderBy: { createdAt: "asc" } },
        },
      },
    },
  })
  ...
  return abonos.map((a) => ({
    ...
    accountingRecordId: a.accountingRecordId,
    paymentProofs: a.accountingRecord?.paymentProofs ?? [],
  }))
}
...
export async function listLinkableEgresos(): Promise<AbonoRow[]> {
  ...
  const records = await prisma.accountingRecord.findMany({
    where: { userId: user.id, category: { in: ["Egreso", "Retención en la fuente"] }, abono: null },
    orderBy: { date: "desc" },
    include: { paymentProofs: { select: { id: true, fileName: true }, orderBy: { createdAt: "asc" } } },
  })
...
export async function extractInvoiceFromPdf(formData: FormData): Promise<InvoiceExtractionResult> {
  ...
  const blank = {
    ...
    contractorName: null, contractorIdNumber: null,
...
export async function updateInvoice(
  id: string,
  data: { contractorId?: string; date?: string; concept?: string; totalAmount?: number; retentionRate?: number | null; notes?: string | null }
): Promise<{ error: string | null }> {
  const user = await requireUser()
