  ...
  const handleUpload = useCallback(
    async (file: File) => {
      ...
      if (file.size > maxSize) {
        setUploadError(`Archivo demasiado grande (${(file.size / 1024 / 1024).toFixed(1)}MB). Máximo: 4.5MB`)
        setUploading(false)
        return
      }
      ...
      try {
        ...
        if (!contentType?.includes("application/json")) {
          throw new Error("Error del servidor - revisa los logs de Vercel. El archivo puede ser demasiado grande o el procesamiento superó el tiempo límite.")
        }
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        if (!res.ok) {
          const err = await res.json().catch(() => ({ error: "Error en el chat" }))
          setMessages((prev) =>
            prev.map((m) =>
    ...
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Sube documentos y chatea con ellos usando Gemini + Embeddings 2.0
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4" style={{ height: "calc(100vh - 480px)" }}>
        {/* Document Panel */}
    ...
    <div className="flex items-start gap-2 p-2 rounded-md border bg-card hover:bg-muted/30 transition-colors group">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
          ...
          {doc.pageCount && (
            <span className="text-xs text-muted-foreground">{doc.pageCount} páginas</span>
          )}
          {doc.status === "ready" && doc._count.chunks > 0 && (
            <span className="text-xs text-muted-foreground">{doc._count.chunks} fragmentos</span>
          )}
        </div>
      ...
      <div className="flex items-start gap-2">
        ...
        <div>
          ...
          <p className="text-yellow-700 dark:text-yellow-500 mt-1">
            Agrega estas variables a tu archivo{" "}
