...
import { Pencil, Trash2, Paperclip, Eye, Loader2, MoreHorizontal, X, Download } from "lucide-react"
              ...
              <td className="px-4 py-2 text-right">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onSelect={() => setToEdit(c)}>
                      <Pencil className="h-4 w-4" />
                      Editar
                    </DropdownMenuItem>
                    <DropdownMenuItem onSelect={() => handleRutClick(c)} disabled={uploadingRutId === c.id}>
                      {uploadingRutId === c.id
                        ? <Loader2 className="h-4 w-4 animate-spin" />
                        : c.rutFileUrl ? <Eye className="h-4 w-4" /> : <Paperclip className="h-4 w-4" />}
                      {c.rutFileUrl ? "Ver RUT" : "Adjuntar RUT"}
                    </DropdownMenuItem>
                    {c.rutFileUrl && (
                      <DropdownMenuItem
                        onSelect={() => handleDeleteRut(c)}
