...
export async function updateAccountingRecord(id: string, field: string, value: string): Promise<{ error: string | null }> {
  const session = await auth()
  if (!session?.user?.email) return { error: null }
  const user = await prisma.user.findUnique({ where: { email: session.user.email } })
  if (!user) return { error: null }

  const allowed = ["concept", "date", "income", "expenses", "category", "property", "cashReceiptGenerated", "expenseVoucherGenerated", "accountId"]
  if (!allowed.includes(field)) return { error: null }
  ...
  switch (field) {
    ...
    case "date": {
      ...
      if (isNaN(d.getTime())) return { error: null }
      data.date = d
      break
    }
    case "income":
    case "expenses": {
      ...
      if (value !== "" && isNaN(n!)) return { error: null }
      // A child record (Egreso explaining a Retiro en Efectivo, or a split payment) can never
      // explain more than its parent's own amount.
      if (n != null) {
        const record = await prisma.accountingRecord.findUnique({ where: { id, userId: user.id }, select: { parentRecordId: true } })
        if (record?.parentRecordId) {
          const pending = await getRecordPendingAmount(record.parentRecordId, id)
          if (pending != null && n > pending) {
            return { error: `El monto excede lo pendiente por explicar ($${pending.toLocaleString("es-CO")}).` }
          }
        }
      }
      data[field] = n
...
export async function generateCashReceipt(
  ...
  const record = await prisma.accountingRecord.findUnique({
    ...
    include: { cashReceipts: { orderBy: { number: "asc" } }, account: true, childRecords: { select: { id: true } } },
  })
  if (!record) return { receipt: null, error: "Registro no encontrado" }

  if (record.childRecords.length > 0) {
    return { receipt: null, error: "Este pago ya fue dividido — generá el recibo desde cada registro hijo." }
  }
  ...
  if (record.cashReceipts.length > 0) {
    ...
    return { receipt: buildReceiptData(first, record, recipientName), error: null }
  }
  ...
  return { receipt: buildReceiptData(created, record, recipientName), error: null }
}
...
export async function generateCashReceiptsBulk(
  ...
  const records = await prisma.accountingRecord.findMany({
    ...
    include: { cashReceipts: true, childRecords: { select: { id: true } } },
  })
  ...
  const toGenerate = ordered.filter((r) => CASH_RECEIPT_CATEGORIES.has(r.category ?? "") && r.cashReceipts.length === 0 && r.childRecords.length === 0)
...
export async function getCashReceiptLiveData(cashReceiptId: string): Promise<CashReceiptData | null> {
  ...
  return buildReceiptData(cr, record, recipientName)
}
...
export async function uploadPaymentProof(
  ...
  if (!ALLOWED_ATTACHMENT_EXTENSIONS.includes(ext)) {
    return { error: "El comprobante debe ser PDF, JPG o PNG." }
  }
  ...
  try {
    ...
    await prisma.paymentProof.create({
      data: { fileName: file.name, fileUrl, accountingRecordId },
    })
    revalidatePath("/accounting")
    revalidatePath("/invoices")
    return { error: null }
...
export async function deletePaymentProof(paymentProofId: string): Promise<{ error: string | null }> {
  const session = await auth()
  if (!session?.user?.email) return { error: "No autorizado" }
  const user = await prisma.user.findUnique({ where: { email: session.user.email } })
  if (!user) return { error: "No autorizado" }
  ...
  const proof = await prisma.paymentProof.findFirst({
    where: { id: paymentProofId, accountingRecord: { userId: user.id } },
  })
  if (!proof) return { error: "Comprobante no encontrado" }

  await deletePaymentProofFile(proof.fileUrl).catch(console.error)
  await prisma.paymentProof.delete({ where: { id: paymentProofId } })
  revalidatePath("/accounting")
  revalidatePath("/invoices")
  return { error: null }
}
...
// ---- Desglose/división de registros (Retiro en Efectivo → Egresos, pagos divididos) ----

// How much of a parent record is still unexplained by its children. `excludeChildId` lets an
// edit to an existing child exclude its own (about-to-be-replaced) amount from the running
// total. Null if the parent itself doesn't exist.
async function getRecordPendingAmount(parentRecordId: string, excludeChildId?: string): Promise<number | null> {
  const parent = await prisma.accountingRecord.findUnique({
    where: { id: parentRecordId },
    include: { childRecords: { select: { id: true, income: true, expenses: true } } },
  })
  if (!parent) return null
  const parentAmount = Number(parent.income ?? parent.expenses ?? 0)
  const explained = parent.childRecords
    .filter((c) => c.id !== excludeChildId)
    .reduce((sum, c) => sum + Number(c.income ?? c.expenses ?? 0), 0)
  return parentAmount - explained
}

export async function createWithdrawalBreakdownRecord(
  parentRecordId: string,
  data: { concept: string; date: string; expenses: string }
): Promise<{ error: string | null }> {
  const session = await auth()
  if (!session?.user?.email) return { error: "No autorizado" }
  const user = await prisma.user.findUnique({ where: { email: session.user.email } })
  ...
  if (isNaN(expenses) || expenses <= 0) return { error: "El monto debe ser mayor a cero." }

  const withdrawal = await prisma.accountingRecord.findUnique({ where: { id: parentRecordId, userId: user.id } })
  if (!withdrawal) return { error: "Registro no encontrado." }

  const pending = await getRecordPendingAmount(parentRecordId)
  if (pending != null && expenses > pending) {
    return { error: `El monto excede lo pendiente por explicar ($${pending.toLocaleString("es-CO")}).` }
  }
  ...
  await prisma.accountingRecord.create({
    data: {
      ...
      date,
      expenses,
      // A Retiro en Efectivo/Egreso breakdown documents "what the cash was spent on" — always a
      // plain Egreso. A Retención en la fuente breakdown instead splits the same retention across
      // several cuentas de cobro, so each piece needs to stay "Retención en la fuente" itself to
      // be linkable to its own invoice.
      category: withdrawal.category === "Retención en la fuente" ? "Retención en la fuente" : "Egreso",
      conceptConfirmed: true,
      categoryConfirmed: true,
      // Inherit the parent's account/bank-statement context — but never bankMovementId, which
      // is unique per record and stays exclusive to the actual bank movement it came from.
      accountId: withdrawal.accountId,
      bankStatementId: withdrawal.bankStatementId,
      parentRecordId,
      userId: user.id,
    },
  })
...
export async function createPaymentSplitRecord(
  parentRecordId: string,
  data: { concept: string; date: string; income: string }
): Promise<{ error: string | null }> {
  const session = await auth()
  if (!session?.user?.email) return { error: "No autorizado" }
  const user = await prisma.user.findUnique({ where: { email: session.user.email } })
  ...
  if (isNaN(income) || income <= 0) return { error: "El monto debe ser mayor a cero." }

  const parent = await prisma.accountingRecord.findUnique({ where: { id: parentRecordId, userId: user.id } })
  if (!parent) return { error: "Registro no encontrado." }

  const pending = await getRecordPendingAmount(parentRecordId)
  if (pending != null && income > pending) {
    return { error: `El monto excede lo pendiente por explicar ($${pending.toLocaleString("es-CO")}).` }
  }
  ...
  await prisma.accountingRecord.create({
    data: {
      ...
      conceptConfirmed: true,
      categoryConfirmed: true,
      propertyConfirmed: parent.property != null,
      // Inherit the parent's account/bank-statement context — but never bankMovementId, which
      // is unique per record and stays exclusive to the actual bank movement it came from.
      accountId: parent.accountId,
      bankStatementId: parent.bankStatementId,
      parentRecordId,
      userId: user.id,
    },
  })
