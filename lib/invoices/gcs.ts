...
export async function uploadInvoiceFile(buffer: Buffer, filename: string, mimeType = "application/pdf"): Promise<string> {
  const bucket = bucketName()
  const gcsPath = `cuentas-cobro/${Date.now()}-${filename}`
  const file = getClient().bucket(bucket).file(gcsPath)
  await file.save(buffer, { contentType: mimeType, resumable: false })
  return `https://storage.googleapis.com/${bucket}/${gcsPath}`
}

export async function getInvoiceSignedUrl(fileUrl: string, filename: string): Promise<string> {
  const bucket = bucketName()
